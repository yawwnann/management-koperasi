...
function normalizePlaceholdUrl(url: unknown): unknown {
  if (typeof url !== "string") {
    return url;
  }

  // Legacy format: https://placehold.co/200x200/color/text?text=AK (served as SVG)
  // Safe format for Next/Image: https://placehold.co/200x200.png/color/text?text=AK
  return url.replace(
    /^https:\/\/placehold\.co\/(\d+x\d+)(\/.*)$/,
    "https://placehold.co/$1.png$2",
  );
}
...
export function getCurrentUser(): any | null {
  ...
  try {
    ...
    const normalizedPhoto = normalizePlaceholdUrl((parsedUser as any).photo);
    if (normalizedPhoto !== (parsedUser as any).photo) {
      const normalizedUser = { ...parsedUser, photo: normalizedPhoto };
