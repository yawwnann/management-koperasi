...
import { DailyReport, AngkatanReport, SystemSummary, Payment, Withdrawal } from '@/types/api.types';
...
export function getDailyReport(date?: string): DailyReport {
  ...
  const dayPayments = MOCK_PAYMENTS.filter((p) => p.createdAt.startsWith(reportDate));
  const dayWithdrawals = MOCK_WITHDRAWALS.filter((w) => w.createdAt.startsWith(reportDate));

  const approvedPayments = dayPayments.filter((p) => p.status === 'APPROVED');
  const approvedWithdrawals = dayWithdrawals.filter((w) => w.status === 'APPROVED');
  const pendingPayments = dayPayments.filter((p) => p.status === 'PENDING');
  const pendingWithdrawals = dayWithdrawals.filter((w) => w.status === 'PENDING');

  const totalPaymentAmount = approvedPayments.reduce((sum, p) => sum + p.amount, 0);
  const totalWithdrawalAmount = approvedWithdrawals.reduce((sum, w) => sum + w.amount, 0);
    ...
    return [
      {
        ...
        averageSavings: angkatanUsers.length > 0 ? totalSavings / angkatanUsers.length : 0,
      },
    ];
  }
